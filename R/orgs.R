organizations       <- function(ctx, user)         api.get.request(ctx, c("users", user, "orgs"))
my.organizations    <- function(ctx)               api.get.request(ctx, c("user", "orgs"))
organization        <- function(ctx, org)          api.get.request(ctx, c("orgs", org))
update.organization <- function(ctx, org, content) api.patch.request(ctx, c("orgs", org), body=content)

# members
organization.members             <- function(ctx, org)       api.get.request(ctx, c("orgs", org, "members"))
...
organization.public.members      <- function(ctx, org)       api.get.request(ctx, c("orgs", org, "public_members"))
...
# teams
teams                       <- function(ctx, org)             api.get.request(ctx, c("orgs", org, "teams"))
team                        <- function(ctx, id)              api.get.request(ctx, c("teams", id))
...
team.members                <- function(ctx, id)              api.get.request(ctx, c("teams", id, "members"))
...
team.repositories           <- function(ctx, id)              api.get.request(ctx, c("teams", id, "repos"))
