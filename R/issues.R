...
all.my.issues       <- function(ctx, ...) api.get.request(ctx, c("issues"), params=.rest(...))
my.issues           <- function(ctx, ...) api.get.request(ctx, c("user", "issues"), params=.rest(...))
organization.issues <- function(ctx, org, ...) api.get.request(ctx, c("orgs", org, "issues"), params=.rest(...))
...
# events
issue.events            <- function(ctx, owner, repo, issue.number) api.get.request(ctx, c("repos", owner, repo, "issues", issue.number, "events"))
...
# labels
repository.labels <- function(ctx, owner, repo)                api.get.request(ctx, c("repos", owner, repo, "labels"))
repository.label  <- function(ctx, owner, repo, name)          api.get.request(ctx, c("repos", owner, repo, "labels", name))
...
milestones       <- function(ctx, owner, repo)                            api.get.request(ctx, c("repos", owner, repo, "milestones"))
milestone        <- function(ctx, owner, repo, milestone.number)          api.get.request(ctx, c("repos", owner, repo, "milestones", milestone.number))
